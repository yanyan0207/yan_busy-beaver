...
fn csv_reads_named_column_and_writes_unresolved_rows() {
    ...
    fs::write(&input, "note,pattern\n\"comma, quoted\",0RA 0RA\nsecond,0LA 0LA\n").unwrap();
    let output = root.join("out");
    let result = Command::new(env!("CARGO_BIN_EXE_bb03_linear_growth"))
        .args(["--csv"]).arg(&input).arg("--output-dir").arg(&output)
        .args(["--max-steps", "0", "--debug", "false"]).output().unwrap();
    ...
    assert_eq!(csv::Reader::from_path(output.join("decided.csv")).unwrap().records().count(), 0);
    // Reject reuse of an output as input before it can be truncated.
    let saved = fs::read(output.join("unresolved.csv")).unwrap();
    let result = Command::new(env!("CARGO_BIN_EXE_bb03_linear_growth"))
        .arg("--csv").arg(output.join("unresolved.csv"))
        .arg("--output-dir").arg(&output).output().unwrap();
